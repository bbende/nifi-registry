...
public class RegistryService {
    ...
    public Set<Bucket> getBuckets(final QueryParameters queryParameters) {
        readLock.lock();
        try {
            final Set<BucketEntity> buckets = metadataService.getBuckets(queryParameters);
            return buckets.stream().map(b -> DataModelMapper.map(b, false)).collect(Collectors.toCollection(LinkedHashSet::new));
        } finally {
            readLock.unlock();
        }
    }
    ...
    public Set<BucketItem> getBucketItems(final QueryParameters queryParameters) {
        readLock.lock();
        try {
            final Set<BucketItem> bucketItems = new LinkedHashSet<>();
            for (final BucketItemEntity itemEntity : metadataService.getBucketItems(queryParameters)) {
                addBucketItem(bucketItems, itemEntity);
            }
    ...
    public Set<BucketItem> getBucketItems(final QueryParameters queryParameters, final String bucketIdentifier) {
        if (bucketIdentifier == null) {
            throw new IllegalArgumentException("Bucket Identifier cannot be null");
        }
        ...
        try {
            ...
            final Set<BucketItem> bucketItems = new LinkedHashSet<>();
            for (final BucketItemEntity itemEntity : metadataService.getBucketItems(queryParameters, bucket)) {
                addBucketItem(bucketItems, itemEntity);
            }
    ...
    private void addBucketItem(final Set<BucketItem> bucketItems, final BucketItemEntity itemEntity) {
    ...
    public Set<VersionedFlow> getFlows(final QueryParameters queryParameters) {
        readLock.lock();
        try {
            // return non-verbose set of all flows
            final Set<FlowEntity> flows = metadataService.getFlows(queryParameters);
            return flows.stream().map(f -> DataModelMapper.map(f, false)).collect(Collectors.toCollection(LinkedHashSet::new));
        } finally {
            readLock.unlock();
        }
    }

    public Set<VersionedFlow> getFlows(final String bucketId) {
